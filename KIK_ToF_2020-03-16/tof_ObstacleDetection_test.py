...
class ObstacleDetection(object):
    def __init__(self, cell_length=20, grid_length=20, min_obstacle_height=50, flag_3d_filter=False):
        """
        ...
        self.Ground_Threshold_Grids = int(math.ceil(self.Ground_Threshold / self.TOF_Grid_Unit_Length))  # grid
        print(self.Ground_Threshold_Grids)

        # 有效体素内的点云的最小数目
        self.Min_Point_Number = 2  # point
        ...
        self.flag_3d_filter = flag_3d_filter
    ...
    def run(self, depth_uint16, filename=None):
        ...
        # --- 以下两种滤波均失败
        # voxel_count = ndimage.gaussian_filter(voxel_count, 3)
        # voxel_count = ndimage.median_filter(voxel_count, 3)

        # ----- 3D 滤波
        if self.flag_3d_filter:
            obstacle_mask_3d = np.zeros(voxel_count.shape, dtype=np.float32)
            obstacle_mask_3d[voxel_count > 0] = 1
            #
            obstacle_mask_3d = ndimage.gaussian_filter(obstacle_mask_3d, 3)
            voxel_count[obstacle_mask_3d <= 0.1] = 0
            #
            # obstacle_mask_3d = ndimage.median_filter(obstacle_mask_3d, 3)
            # voxel_count[obstacle_mask_3d <= 0] = 0
        ...
        for _idx in range(grid_size[0]):
            ...
            if idx_arr[1].size > 0:
                ...
                if flag_viz_obstacle:  # 障碍物较多时，该过程耗时
                    ...
                    # ------------------------------------------------------------------------------
                    # ---- 为了可视化障碍时，便于观察小障碍，将障碍物进行扩大, 在障碍物周围padding _er个体素
                    # _er = 0
                    _er = 1
...
if __name__ == '__main__':
    ...
    def get_rgbd_filepaths(root):
        d_filepaths = glob.glob(root + '/*.png')  # List all files in data folder
        d_filepaths.sort()
        print("Get {} RGBD files in '{}'.".format(len(d_filepaths), root))
        return d_filepaths

    time_start = datetime.datetime.now()

    # fpaths = 'C:/Users/LiJie/Documents/kik/kik_tof_UnloadingSpace/tof_20200302/tof_data/6cm/'
    # fpaths = 'C:/Users/LiJie/Documents/kik/kik_tof_UnloadingSpace/tof_20200302/tof_data/8cm/'
    # fpaths = 'C:/Users/LiJie/Documents/kik/kik_tof_UnloadingSpace/tof_20200302/tof_data/10cm/'
    # fpaths = 'C:/Users/LiJie/Documents/kik/kik_tof_UnloadingSpace/tof_20200302/tof_data/10cm2/'
    ...
    # ---- test: 读取录制到磁盘的Depth和RGB图像序列
    RGBD_filepaths = get_rgbd_filepaths(fpaths)
    ...
    min_obstacle_height = 60  # 单位mm, 实际对应在栅格中的高度为： min_obstacle_height/cell_length的向上取整
    m_tof_obstacle_detection = ObstacleDetection(cell_length, grid_length, min_obstacle_height, True)

    for RGBD_filename in RGBD_filepaths:
        print(RGBD_filename)
        RGBD_img = imageio.imread(RGBD_filename)
        RGBD_img_copy = RGBD_img.copy()
        ...
        # --- 滤波：可用中值滤波。不能采用高斯滤波，不能采用平滑滤波，双边滤波不支持16位图像
        # [x] input_depth_uint16 = cv2.GaussianBlur(input_depth_uint16, (3, 3), 0) #
        # [x] input_depth_uint16 = cv2.blur(input_depth_uint16, (3, 3))  #
        # [x] input_depth_uint16 = cv2.bilateralFilter(input_depth_uint16,5,21,21)  #
        # input_depth_uint16 = cv2.medianBlur(input_depth_uint16, 3)  # 中值滤波
        # input_depth_uint16 = cv2.medianBlur(input_depth_uint16, 3)  # 中值滤波

        filename = None
        #filename = RGBD_filename[:-4] + '_{}-{}mm.ply'.format(cell_length, min_obstacle_height)
        # filename = RGBD_filename[:-4] + '_{}_{}_{}.ply'.format(cell_length, grid_length, min_obstacle_height)
        ...
        #cv2.imshow('RGBD_img', RGBD_img_copy)
